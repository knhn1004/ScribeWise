...
class Config:
    ...
    def setup(cls):
        ...
        if cls.LLM_MODEL not in GROQ_TEXT_MODELS:
            print(f"Warning: LLM_MODEL {cls.LLM_MODEL} not found in available Groq text models. Using default: {DEFAULT_TEXT_MODEL}")
            cls.LLM_MODEL = DEFAULT_TEXT_MODEL
            
        if cls.STT_MODEL not in GROQ_SPEECH_MODELS:
            print(f"Warning: STT_MODEL {cls.STT_MODEL} not found in available Groq speech models. Using default: {DEFAULT_SPEECH_MODEL}")
    ...
    def check_optional_features(cls) -> Dict[str, bool]:
        ...
        return {
            ...
            "translation": cls.STT_MODEL in ["whisper-large-v3"] and cls.GROQ_API_KEY is not None
        }
