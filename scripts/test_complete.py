    ...
    try:
        ...
        for output_type, output_path in outputs.items():
            if output_type != "error":
                ...
                print(f"- {output_type}: {output_path} ({'✓' if file_exists else '✗'}, {file_size} bytes)")
        ...
        if "url" in result["video_info"] and hasattr(result["video_info"]["url"], "__str__"):
            result["video_info"]["url"] = str(result["video_info"]["url"])
        
        result_path = f"{OUTPUT_DIR}/{video_result['video_info']['video_id']}_test_result.json"
