...
class PDFService:
    ...
    async def extract_text_from_pdf(self, pdf_path: str) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Extracted {len(full_text)} characters from {metadata['page_count']} pages in {pdf_path}"
            )
    ...
    async def save_intermediate_results(
        self, pdf_id: str, chunk_results: List[Dict], result_type: str
    ) -> str:
    ...
    async def combine_flashcards(self, chunk_flashcards: List[Dict]) -> Dict:
        ...
        logger.info(
            f"Combined and deduplicated flashcards: {len(unique_cards)} unique cards from {len(all_cards)} total"
        )
    ...
    async def process_pdf(self, pdf_path: str, summarization_service) -> Dict[str, Any]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_result = await summarization_service.process_content_for_chunk(
                chunk_content
            )
